...
export const useMutateTask = () => {
  ...
  const updateTaskMutation = useMutation(async (task: EditedTask) => {
    const { data, error } = await supabase.from("todos").update({ title: task.title}).eq("id", task.id).select()
    if(error) throw new Error(error.message)
    return data
  }, {
  ...
  const deleteTaskMutation = useMutation(async (id: string) => {
    ...
    onSuccess: (_, variables) => {
      const previousTodos = queryClient.getQueryData<Task[]>(["todo"])
      if(previousTodos){
        queryClient.setQueryData(["todos"], previousTodos.filter((task) => task.id !== variables))
      }
      reset()
    },
    onError: (err: any) => {
